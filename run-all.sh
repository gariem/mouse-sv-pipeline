...
FILTER_VALUES="$(cat run-params.txt | grep 'filter_hets=' | cut -d '=' -f 2 )"
...
do
    ...
    do 
        ...
        do 
            ...
            do
                ...
                do 
                    echo "nextflow run taks/merge_survivor_mapped.nf --strain $STRAIN --max_dist $MAX_DIST --min_callers $MIN_CALLERS --min_size $MIN_SIZE --filter_hets $FILTER" >> $CMD_FILE
                    echo "nextflow run taks/merge_survivor_simple.nf --strain $STRAIN --max_dist $MAX_DIST --min_callers $MIN_CALLERS --min_size $MIN_SIZE --filter_hets $FILTER" >> $CMD_FILE
                done
            done
        done
    done
    ...
    do 
        echo "nextflow run taks/merge_combi.nf --strain $STRAIN --min_coverage $MIN_COVERAGE" >> $CMD_FILE
    done
    ...
    do
        ...
        do 
            ...
            for MIN_COVERAGE in $(echo $COMBI_MIN_COVERAGE_VALUES | sed "s/,/ /g")
            do
                echo "nextflow run taks/metrics_collect.nf --intersect_window_a $WINDOW --intersect_window_b $WINDOW --filter_hets $FILTER --input_files ./data/merged/$STRAIN-combisv_c$MIN_COVERAGE.vcf" >> $CMD_FILE
            done
        done
        ...
        do 
            ...
            do 
                for MIN_CALLERS in $(echo $SURVIVOR_MIN_CALLERS_VALUES | sed "s/,/ /g")
                do
                    echo "nextflow run taks/metrics_collect.nf --intersect_window_a $WINDOW --intersect_window_b $WINDOW --input_files ./data/merged/$STRAIN-survivor_${MAX_DIST}_${MIN_CALLERS}_${MIN_SIZE}.vcf" >> $CMD_FILE
                    echo "nextflow run taks/metrics_collect.nf --intersect_window_a $WINDOW --intersect_window_b $WINDOW --input_files ./data/merged/$STRAIN-survivor_${MAX_DIST}_${MIN_CALLERS}_${MIN_SIZE}_nohets.vcf" >> $CMD_FILE

                    for FILTER in $(echo $FILTER_VALUES | sed "s/,/ /g")
                    do 
                        echo "nextflow run taks/metrics_collect.nf --intersect_window_a $WINDOW --intersect_window_b $WINDOW --filter_hets $FILTER --input_files ./data/merged/$STRAIN-unmapped_${MAX_DIST}_${MIN_CALLERS}_${MIN_SIZE}.vcf" >> $CMD_FILE
                    done
                done
            done
        done
